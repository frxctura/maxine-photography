    ...
    for root, _, files in os.walk(image_directory):
        for file in files:
            if any(file.lower().endswith(ext) for ext in extensions):
                ...
                full_image_url = '/photos/' + os.path.relpath(full_image_path, image_directory).replace('\\', '/')
                thumbnail_image_url = '/thumbnails/' + os.path.relpath(thumbnail_image_path, thumbnail_directory).replace('\\', '/')
